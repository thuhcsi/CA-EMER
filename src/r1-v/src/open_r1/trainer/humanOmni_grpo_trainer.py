...
from transformers import BertTokenizer, SiglipImageProcessor, WhisperFeatureExtractor
...
from humanomni.mm_utils import tokenizer_multimodal_token, process_video, process_audio
...
class HumanOmniVLGRPOTrainer(Trainer):
    ...
    def __init__(
       ...
       # import ipdb;ipdb.set_trace()
        ...
        if isinstance(model, str):
            ...
            for p in model.get_audio_tower().parameters():
                p.requires_grad = False
        ...
        if processing_class is None:
            processing_class = AutoProcessor.from_pretrained("YOUR_PATH/Qwen/Qwen2-VL-2B-Instruct")
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # === 1) collect prompts & bert_prompts ===
        prompts = []
        bert_prompts_texts = []
        for x in inputs:
            prompt = x["prompt"]
            bert_prompts_texts.append(prompt[0]['content'][1]['text'])
            # move original video text into video field and add markers
            text = prompt[0]["content"][0].pop('text')
            video_path = x["video"]
            prompt[0]["content"][0]["video"] = video_path
            prompt[0]['content'][1]['text'] = '<vi_start><video><vi_end>\n<au_start><audio><au_end>\n' + prompt[0]['content'][1]['text']
            prompts.append(prompt)

        # Tokenize bert prompts (BERT tokenizer) -> dict of tensors (input_ids, attention_mask, ...)
        bert_prompts = self.bert_tokenizer(bert_prompts_texts, return_tensors='pt', padding=True, truncation=True, add_special_tokens=True)
        # Move bert_prompts tensors to device
        for k in bert_prompts:
            bert_prompts[k] = bert_prompts[k].to(device)

        # === 2) make plain text prompts for multimodal tokenizer ===
        prompts_text = []
        for example in inputs:
            prompt_text = maybe_apply_chat_template(example, self.processing_class)["prompt"]
            prompts_text.append(prompt_text)

        # === 3) tokenize with tokenizer_multimodal_token and pad properly ===
        ids_list = []
        for prompts_text_ in prompts_text:
            ids = tokenizer_multimodal_token(prompts_text_, self.processing_class.tokenizer, '<video>', return_tensors='pt')
            # handle returned shapes: ensure 1D tensor of token ids
            if isinstance(ids, dict):
                # if tokenizer_multimodal_token returns a dict, adjust accordingly
                # assume key 'input_ids' exists
                ids = ids.get('input_ids', ids[next(iter(ids))])
            ids = ids.to(device)
            if ids.dim() == 2 and ids.size(0) == 1:
                ids = ids.squeeze(0)
            ids_list.append(ids)

        # pad_sequence -> (B, P_max)
        ...
        # === 4) load and stack video/audio per-sample (ensure shapes (B, ...)) ===
        video_tensors = []
        audio_tensors = []
        for prompt in prompts:
            video_file = prompt[0]["content"][0]["video"]

            # process_video should return a tensor per sample, e.g. (num_frames, C, H, W)
            video_ids = process_video(video_file, self.visual_processor, aspect_ratio="pad", num_frames=8)
            # ensure video_ids is tensor; move to device later when stacking
            video_tensors.append(video_ids)

            # process_audio returns (audio_array, sr), then self.audio_processor -> dict with 'input_features'
            audio_np, audio_sample_rate = process_audio(video_file)
            audio_feat = self.audio_processor(audio_np, sampling_rate=audio_sample_rate, return_tensors='pt')['input_features']
            # audio_feat expected shape often (n_chunks, feat, time) or (feat, time) or (1, feat, time)
            # normalize to 2D/3D below after stacking
            audio_tensors.append(audio_feat.squeeze(0) if audio_feat.dim() == 3 and audio_feat.size(0) == 1 else audio_feat)

        # stack -> video: (B, num_frames, C, H, W)
        video = torch.stack(video_tensors, dim=0).to(device)

        # stack audios -> could be (B, n_chunks, feat, time) or (B, feat, time)
        audios = torch.stack(audio_tensors, dim=0)
        # if audios have chunk dim (4D), aggregate chunks to (B, feat, time)
        if audios.dim() == 4:
            # conservative aggregation: mean over chunk dimension
            audios = audios.mean(dim=1)
        audios = audios.to(device)

        # === 5) attention masks and build prompt_inputs ===
        attention_masks = input_ids.ne(pad_id).long().to(device)

        prompt_inputs = {
            "inputs": input_ids,             # (B, P)
            "images": video,                 # (B, num_frames, C, H, W)
            "attention_mask": attention_masks,
            "prompts": bert_prompts,         # dict with tensors already on device
            "audios": audios,                # (B, feat, time)
        }
